...
- dig の公開入口は `Claude: /dig` / `Codex: $dig` / `OpenCode: /dig`
- runtime adapter の実体ファイルは `dig-core` / `dig-claude` / `dig-codex` / `dig-opencode`
- Codex / OpenCode へ同期するトップレベル skill は公開入口だけに絞り、adapter は `~/cursor/devkit` 配下の shared source から内部参照する
...
| [Git](https://git-scm.com/) | `~/cursor/devkit` の checkout 更新 | 公式サイトからインストール | `git --version` |
...
- 共通 DevKit source: `~/cursor/devkit`
...
Codex / OpenCode は `~/cursor/devkit` の checkout を共通 source として使い、skills / commands / helper / templates をそこから同期する。Codex の project 専用 skill は各 repo の `.agents/skills` に置き、Claude の project 専用 skill は `.claude/skills` に置く。`~/.codex/skills` は user-managed skill root ではない。
...
初回だけは checkout または Marketplace 配下の script を直接実行する。`~/cursor/devkit` が無ければ、この実行で clone を作る。
...
- `~/cursor/devkit` の shared source から Codex / OpenCode の DevKit 管理 user-level assets を再同期
