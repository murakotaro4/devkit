...
- 新しい `/dig` は Claude 専用のオーケストレーションスキル。深掘りインタビュー（AskUserQuestion 繰り返し）→ 調査・計画 → 計画承認 → 実装 backend 選択 → 実装委譲 → 親エージェントの diff 自レビュー + テスト → 修正ループ → 報告、という流れで動く
- ユーザー確認ポイントは計画承認と backend 選択（`codex exec` / Claude サブエージェント）のみ
...
Codex CLI は任意。`/dig` の実装委譲 backend、および diff レビューのセカンドオピニオン（`codex -a never exec review --uncommitted`）に使える。
未インストールでも作業は進められ、その場合はレビューを親エージェントの自レビュー + テストで完結させる。
詳細は「レビュー方針」セクション参照。

| ツール | 用途 | インストール | 確認コマンド |
|--------|------|-------------|-------------|
| [Codex CLI](https://github.com/openai/codex) | `/dig` 実装 backend / レビューのセカンドオピニオン | `npm install -g @openai/codex` | `codex --version` |
...
- 深掘り + 実装委譲: `/dig`（親 Claude = 深掘りインタビュー・計画・diff レビュー・テスト、backend = 実装のみ。backend は `codex exec` か Claude サブエージェント[Sonnet/Haiku] から選ぶ。`codex exec` のモデル / effort は `~/.codex/config.toml` の既定に従い、effort は選択可）
...
- 変更が大きい場合や別視点が欲しい場合は、任意で `codex -a never exec review --uncommitted` のセカンドオピニオンを取る
- Codex CLI が無い環境でも、自レビュー + テストでレビューを完結できる
...
見つからない場合はセカンドオピニオンを省略し、親エージェントの自レビュー + テストでレビューを完結させる。
