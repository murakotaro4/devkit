...
- `goal-prompt`: 不在実行に耐えるレビュー済みゴールプロンプト + 起動プロンプトを作る skill。既定成果物は本文全文を含むインライン 1 ブロックで、ファイルが必要な形態だけ `.claude/goal-runs/` へ保存して参照起動プロンプトを提示する。実行はユーザーの 1 アクションに分離し、ゴール本文には同領域への完了レポート書き出しを焼き込んで実行後の検収の界面にする
