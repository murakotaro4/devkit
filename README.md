...
- `dig`: `/dig` として深掘り、計画、実装委譲、diff review、検証を扱うオーケストレーション skill。成果物は worktree 上で実装し、統合(直接統合の merge / push、または PR の提出 + CI green 確認 + merge)まで完了した変更。実装系で計画がまだ無い場合は `dig` が正道で、承認済み計画はゴール化引き継ぎで `goal-prompt` へ渡せる
