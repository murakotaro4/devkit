...
DevKit は Claude Code / Codex 向けの個人プラグインです。配布する skill は `dig-goal` / `improve-skill` / `setup` / `refactor` / `memory-review` / `handoff` / `backlog` / `catch-up` / `commit-push` の 9 本にし、導入と更新は marketplace を正本にします。Node statusline も plugin に同梱します。
...
- `setup`: 対象リポジトリへの DevKit ルール同期、環境前提チェック(claude / codex / cursor-agent / node / python3)、thought-db 接続と updater のユーザー環境同期、旧 updater 名の残骸 prune、statusline 適用、Windows Terminal の UDEV Gothic NF(GitHub release からのダウンロード)適用を扱う skill
