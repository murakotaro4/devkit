...
DevKit は Claude Code / Codex 向けの個人プラグインです。配布する skill は `dig` / `improve-skill` / `setup` / `refactor` / `memory-review` / `goal-prompt` / `handoff` / `backlog` / `catch-up` / `commit-push` の 10 本にし、導入と更新は marketplace を正本にします。v7 では Node statusline も plugin に同梱します。
...
- `dig`: `/dig` として深掘り、計画、実装委譲、diff review、検証を扱うオーケストレーション skill。成果物は worktree 上で実装し、統合(既定は PR の提出 + CI green 確認 + merge。PR 不可 repo では直接統合の merge / push)まで完了した変更。実装系で計画がまだ無い場合は `dig` が正道で、承認済み計画はゴール化引き継ぎで `goal-prompt` へ渡せる
- `improve-skill`: skill 改善の調査、設計、レビュー、更新を扱う skill
- `setup`: 対象リポジトリへの DevKit ルール同期、環境前提チェック(claude / codex / cursor-agent / node / python3)、thought-db 接続と updater のユーザー環境同期、旧 updater 名の残骸 prune、statusline 適用、Windows Terminal の JetBrainsMono Nerd Font 適用を扱う skill
...
- `goal-prompt`: 不在実行に耐えるゴールプロンプトを組み立て、独立レビュー後、既定では同一セッションがそのまま自律実行して完了レポートまで完遂する skill。親は実行移行前に本文を `.claude/goal-runs/` へ保存して圧縮復帰点にする。起動プロンプト提示は、定期実行・別ターミナル・別 PC・後で実行・白紙コンテキスト実行をユーザーが明示した例外形態のみ
...
使い分けの軸は自律度です。対話しながら worktree 上で実装し、統合まで完遂するなら `dig`、判断を前倒ししてレビュー後そのまま同一セッションで自律実行・完遂するなら `goal-prompt`、`dig` で深掘り・計画済みの内容を自律実行へ移すならゴール化引き継ぎを使います。起動プロンプト提示はユーザーが明示した例外形態のみです。

`dig` と `setup` は Claude Code と Codex の両親ハーネスを想定します。Claude Code では既存の対話/承認ツールを使い、Codex では plan mode と組み込み plan / agent 機能へ読み替えます。`refactor` は read-only の計画化で終了し、実装は `/dig` 側へ接続します。`memory-review` の監査は read-only とし、書き込みはレポート保存と承認済み軽微修正だけに限定します。構成変更を伴う大きい修正は `/dig` へ引き継ぎます。`goal-prompt` は実行前のプロンプト authoring と独立レビューを担い、既定では同一セッションの自律実行へ移行して完遂します。起動プロンプトと回収手順の提示で終了するのはユーザーが明示した例外形態のみです。`handoff` は書き出し専用で、読み込み・自動復元・commit / push は行いません。`backlog` は read-only の横断棚卸しで、handoff が書き出した引継ぎや goal-prompt の完了レポートを読む側の工程を担い、実装は `/dig` へ接続します。`catch-up` は外部値の追従専用で、内部メモリ監査は `memory-review`、セッション内エラー起点の改善は `improve-skill retro`、workflow contract の変更は `/dig` を使います。`commit-push` はレビュアー機能を持たない commit / push 専用の工程で、レビューが必要な実装は `/dig` を使います。
