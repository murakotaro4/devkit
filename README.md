...
DevKit は Claude Code / Codex / Cursor 向けの個人開発キットです。配布する skill は `dig-goal` / `improve-skill` / `setup` / `refactor` / `memory-review` / `handoff` / `backlog` / `catch-up` / `commit-push` / `repo-loop` の 10 本にし、Claude Code / Codex の導入と更新は marketplace、Cursor への配布は Claude Code plugin 経由を正本にします。Node statusline も plugin に同梱します。
