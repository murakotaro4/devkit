...
DevKit は Claude Code / Codex / Cursor 向けの個人開発キットです。配布する skill は `dig` / `goal-prompt` / `improve-skill` / `setup` / `refactor` / `memory-review` / `handoff` / `backlog` / `catch-up` / `commit-push` / `repo-loop` の 11 本にし、Claude Code / Codex の導入と更新は marketplace、Cursor への配布は Claude Code plugin 経由を正本にします。Node statusline も plugin に同梱します。

## Migration Notice

v14.0.0 で `dig-goal` を `dig`(深掘りから実装・PR 統合まで完遂する主ワークフロー)と `goal-prompt`(Goal プロンプト保存生成)へ分割しました。旧 `/dig-goal` 表記、および実行形態名の「現セッション自律実行」「起動プロンプト提示」は廃止しました。
...
- `dig`: 深掘り、調査、計画、独立レビュー、worktree 実装、diff レビュー、統合(既定は PR の提出 + CI green 確認 + merge。PR 不可 repo では直接統合)まで完遂する主ワークフロー skill。**既定は実装完遂**
- `goal-prompt`: 会話・仕様・dig のレビュー済み計画から Goal プロンプトを `.claude/goal-runs/` へ保存生成し(上書きせず連番)、`/goal` 起動プロンプトを出力する軽量 skill。コード変更・commit / push・PR 作成はしない
- `improve-skill`: skill 改善の調査、設計、レビュー、更新を扱う skill
- `setup`: 対象リポジトリへの DevKit ルール同期、環境前提チェック(claude / codex / cursor-agent / node / uv)、thought-db 接続・updater・Claude Code compaction env・cursor-agent Git Bash シムのユーザー環境同期、旧 updater 名と Cursor 同期資産の残骸 prune、statusline 適用、Windows Terminal の UDEV Gothic NF(GitHub release からのダウンロード)適用を扱う skill
...
- `plugins/devkit/skills/repo-loop/SKILL.md`: `repo-loop` skill
