...
DevKit は Claude Code / Codex 向けの個人プラグインです。配布する skill は `dig` / `improve-skill` / `setup` / `refactor` の 4 つにし、導入と更新は marketplace を正本にします。v7 では Node statusline も plugin に同梱します。
...
`dig` と `setup` は Claude Code と Codex の両親ハーネスを想定します。Claude Code では既存の対話/承認ツールを使い、Codex では plan mode と組み込み plan / agent 機能へ読み替えます。`refactor` は read-only の計画化で終了し、実装は `/dig` 側へ接続します。
