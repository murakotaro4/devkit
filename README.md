...
置き換え先:
- 入口は `/devkit:dig`（Claude）/`/prompts:devkit-dig`（Codex）/`/dig`（OpenCode）を使用
- runtime adapter は `dig-core` / `dig-claude` / `dig-codex` / `dig-opencode`

移行チェック:
```bash
node plugins/devkit/scripts/check-dig-migration.mjs --mode=repo
```
...
- Claude Code: `/devkit:dig` `/devkit:gpt-pro` `/devkit:deep-research` `/devkit:mermaid-show` `/devkit:amazon-search` `/devkit:improve-skill` `/devkit:codex-search` `/devkit:devkit-init`
