...
- CI: GitHub Actions（`pull_request` + `workflow_dispatch`）で `uv run verify-full` を実行
...
|--------|------|-------------|-------------|
| [Node.js](https://nodejs.org/) / npm | CLIツール基盤・更新コマンド実行 | 公式サイトから LTS 版をインストール | `node -v && npm -v` |
| [Git](https://git-scm.com/) | リポジトリクローン・runtime-local source root の維持 | 公式サイトからインストール | `git --version` |
...
| 環境 | パス形式 | 例 |
|------|---------|---|
| Git Bash | `/c/Users/...` | `/c/Users/murak/.codex/devkit/source` |
| WSL | `/mnt/c/Users/...` | `/mnt/c/Users/murak/.codex/devkit/source` |
| cmd / PowerShell | `C:\Users\...` | `C:\Users\murak\.codex\devkit\source` |
...
各 runtime はこの repo checkout を source root として使い、skills / commands / helper / templates を source root から直接同期する。
...
- Claude Code: `/dig` `/devkit:gpt-pro` `/devkit:deep-research` `/devkit:mermaid-show` `/devkit:improve-skill` `/devkit:codex-search` `/devkit:devkit-init`
...
- Codex の source root が有効なら、helper / template を先に `~/.codex` 側へ再同期する
...
- Codex を外す場合は `~/.codex/devkit/source` と `~/.codex` 配下の DevKit 管理 assets を削除する。
- OpenCode を外す場合は `~/.config/opencode/devkit/source` と `~/.config/opencode` 配下の DevKit 管理 assets を削除する。
...
- アプリ再起動
- runtime-local source root の確認:
  ```bash
  ls -la ~/.config/opencode/devkit/source
