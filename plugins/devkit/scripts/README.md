...
- DevKit source: `~/cursor/devkit`
- Codex の公開 skill: `~/.agents/skills`
- OpenCode の公開 skill: `~/.config/opencode/skills`

skills / commands / helper / templates は `~/cursor/devkit` の DevKit source から同期する。
...
- PowerShell / cmd 版は install 済み script から `~/cursor/devkit` の DevKit source を参照して同期する。

### それ以外のシェル

初回だけは checkout または Marketplace 配下の script を直接実行し、その後はインストール済みの `update-devkit` を使う。`~/cursor/devkit` が無ければ初回実行時に clone する。`update-ccx` でも同じ挙動を呼べる。
