...
DevKit の setup / update / verification scripts を置くディレクトリです。marketplace 配布を正本にし、skill surface は `dig` / `goal-prompt` / `improve-skill` / `setup` / `refactor` / `memory-review` / `handoff` / `backlog` / `catch-up` / `commit-push` / `repo-loop` の 11 本を扱います。
...
- v9 migration marker(`.migrated-v9-dig-goal`) と統合前 live skill directory prune
