...
Orchestrator は委譲と統合のみを担当し、自身では作業しない。

| 役割 | 担当フェーズ | 数 | 備考 |
|------|------------|-----|------|
| Orchestrator | 全体 | 1 | フェーズ進行管理・委譲判断・統合・停止判断のみ |
| Explorer | Phase 2 | 3+（状況に応じて調整） | コードベース探索（Grep/Glob/Read）。並列実行 |
| Researcher | Phase 2 | 3+（状況に応じて調整） | 外部調査（Web検索/ドキュメント/codex-search等）。並列実行 |
| Plan agent | Phase 3 | 1 | 計画作成を Orchestrator から委譲 |
| Eval agent | Phase 4/6 | 1 | codex exec によるレビュー専用。独立評価 |
| Implementer | Phase 5 | 1+ | worktree 分離で並列実行可能 |

- 各 adapter は上記の抽象ロールを runtime 固有ツールにマッピングする
- Phase 2 では Explorer と Researcher を並列起動し、調査を網羅的に行う
- Eval agent はレビュー専用（Phase 4/6）。調査・相談は AGENTS.md の codex exec 相談ルールに従う

```mermaid
flowchart TD
    Start(["/dig 起動"]) --> O["Orchestrator"]

    O --> P1["Phase 1: 要件ヒアリング<br/>質問ツールで深堀り"]
    P1 --> P1c{"チェックリスト完了?<br/>目的/成功条件/制約/非対象/承認"}
    P1c -->|No| P1
    P1c -->|Yes + ユーザー同意| P2

    subgraph P2["Phase 2: 調査（並列）"]
        E1["Explorer ×3+"]
        R1["Researcher ×3+"]
    end
    P2 --> P3

    P3["Phase 3: Plan agent<br/>計画作成・分解・sizing"] --> P4

    subgraph P4["Phase 4: 計画レビュー"]
        EvalP["Eval agent (codex exec)<br/>Path B: 計画書レビュー"]
        EvalP --> G4{"REVIEW_GATE_PLAN<br/>critical=0 && high=0?"}
        G4 -->|Fail| Fix4["修正"] --> EvalP
        G4 -->|Unavailable| Stop4["⛔ PLAN_REVIEW_UNAVAILABLE"]
    end
    G4 -->|Pass| P5

    subgraph P5["Phase 5: 実装（並列）"]
        Impl["Implementer ×N<br/>worktree 分離"]
    end
    P5 --> P6

    subgraph P6["Phase 6: 実装レビュー"]
        EvalS["Eval agent<br/>Path A: REVIEW_GATE_SUBTASK"]
        EvalS --> EvalI["Eval agent<br/>Path A: REVIEW_GATE_INTEGRATION"]
        EvalI --> G6{"critical=0 && high=0?"}
        G6 -->|Fail| Fix6["修正"] --> EvalS
    end
    G6 -->|Pass| P7

    P7["Phase 7: git add → commit → push"] --> Done([完了])
```
...
- 停止時のタスク cleanup: 未完了タスクを取り消す（具体的な手段は adapter が定義）
- 全タスク完了手順（必須）:
  1. 全タスクが完了
  2. コミット前レビュー通過 + `git commit` 成功
  3. 上記2条件を満たした後に全タスクを mark_complete
  4. `git push` は完了の条件に含めない（commit 成功で作業は保全されている）
...
| Path B: 計画書/プロンプトレビュー | `codex exec -m <model> "<prompt>"` | 計画書ファイル、または任意のプロンプト | Phase 4（REVIEW_GATE_PLAN）、Phase 2 調査 | — |
...
| REVIEW_GATE_SUBTASK | Phase 6 各サブタスク後 | 未コミット差分 | small: 変更5行未満 or ドキュメントのみ。medium/large: スキップ不可 |
| REVIEW_GATE_INTEGRATION | Phase 6 全完了後 | 統合 diff | サブタスク1件以下（0件=単純タスク、1件=SUBTASK で済み） |
...
| 2nd | Phase 2 調査 | Explorer ×3+ / Researcher ×3+ の並列起動で委譲 |
...
| file review | `codex exec -m <model> "<prompt>"` + ファイル入力 | ファイルベースのレビュー | Phase 4/6 |
