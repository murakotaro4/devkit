...
- `auto-retro`: **デフォルト動作**。引数なしで起動。会話コンテキストからエラーおよびユーザーフィードバックを自動検出→修正適用→Codexレビュー→コミット
...
1. モード確認のAskUserQuestionは**不要**（デフォルト動作）
...
5. 修正対象は**エラーが発生した、またはユーザーフィードバックが関連するスキルのSKILL.md / CLAUDE.mdのみ**（_coreスクリプトや他スキルへの変更は禁止。読み込みは許可）
6. auto-retro自体が失敗した場合は警告メッセージのみ出力し、呼び出し元ワークフローの成果物には影響しない
7. Plan モード中（dig-codex 等）の場合、Step 3（編集適用・Codex レビュー・コミット）は実行せず、修正提案の提示のみ行う。Step 0-2 は通常通り実行する
...
> 他スキルの最終ステップから自動トリガーされる（引数なしでデフォルト動作）。
> 対象スキルはエラーおよびユーザーフィードバックのコンテキストから自動判定。
