...
親エージェント = 対象 repo に散在する残課題の read-only スキャン、統合、鮮度判定、ダッシュボード提示、dig への引き継ぎ。backlog 自身はファイル変更、実装、実装オーケストレーションを行わない。
...
正本は devkit リポジトリの `AGENTS.md`「スキル共通契約 > 計画・レポートの 2 層提示」。結果はチャットへ提示し、ファイルへは保存しない。カテゴリ別詳細の前に、第 1 層「結論 + 推奨次アクション(3 件以内)」の承認用サマリーを先頭提示し、第 2 層としてカテゴリ別詳細を後段に置く。第 1 層だけで次アクションを判断できるよう、次の 7 カテゴリを省略しない。

1. 何を / なぜ: 1〜2 文
2. 判断してほしい点: 推奨次アクションを付けて最大 3 件。なければ「なし」
...
6. 検証: 何を確認できれば棚卸し結果を採用できるかを 1 行
7. 独立レビュー状態: `適用なし` + backend `適用なし`。一般形の値は `実施済み(指摘 N 件反映)` / `skip(理由)` / `適用なし` の 3 値とし、backlog は read-only のため適用しない
...
実装が必要な項目をユーザーが選んだ場合は、承認された項目を dig の step 2 (調査 + 計画) に渡す計画草案として整形して出力する。詳細な実行契約は `plugins/devkit/skills/dig/SKILL.md` を参照する。backlog 自身はここで終了し、実装・backend 選択・レビュー・修正ループはすべて dig 側のフローで行う。
