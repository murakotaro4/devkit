...
> **Role**: dig-cursor = dig-core 契約を Cursor ツール（AskQuestion / Shell / TodoWrite）にマッピングする adapter

## Plan Mode ↔ Phase マッピング

| モード | 対応フェーズ | 備考 |
|--------|------------|------|
| Plan Mode | Phase 1-4 | 編集・書き込み不可。計画とレビューのみ |
| Agent Mode | Phase 5-7 | Phase 4 通過後に実装・コミットを実行 |
...
- Phase 1 は `AskQuestion` で最低 1 ラウンド（原則 4 問）を実施する。ラウンド数に上限なし
- Phase 1 完了チェックリスト（全項目が確定するまで Phase 2 に進まない）:
  - [ ] 目的 / [ ] 成功条件 / [ ] 制約 / [ ] 非対象 / [ ] 承認（ユーザー同意）
- 完了確認: AskQuestion で「要件は十分固まりましたか？」を必ず確認する
- Phase 4 のレビューゲート通過前に実装へ進まない
- 既存の dig-claude フローを踏襲しつつ、Cursor のツール制約に合わせて同等の停止コードを使う

## Phase 4: 計画レビュー

> **レビュー経路**: Phase 4 は Path A（`review --uncommitted`）を使用。Cursor では plan ファイルが未コミット差分として存在するため、差分レビューで計画をレビューできる。カスタムプロンプト併用不可の制約あり。Phase 6 も同じ Path A を使用する。
...
dig-core のエージェントアーキテクチャを Cursor ツールに写像:
...
- ゲート停止コード: `DIG_CURSOR_*` プレフィックスで定義。パース失敗時は PRIMARY → FALLBACK → 停止
