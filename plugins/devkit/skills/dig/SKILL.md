...
計画レビューを経た(skip 選択時は未実施の)、backend / モデルが明記された計画で承認を得る。通常実装では計画レビュー / 実装 / diff レビューの 3 役を明記する。実装 backend が「ゴール化して自律実行」の場合は diff レビュー backend を明記せず、goal-prompt のゴール本文に実装後レビュー要件を焼き込む旨を計画へ明記する。承認なしで実装に進まない。
