...
argument-hint: "[task]"
allowed-tools: ["AskUserQuestion", "ExitPlanMode", "Bash", "Read", "Grep", "Glob", "Agent", "request_user_input", "spawn_agent", "wait_agent", "send_input", "close_agent", "TaskCreate", "TaskUpdate", "TaskOutput"]
---
...
### 3. backend 選択(選択肢付き質問 1 ラウンド 3 問)

計画承認の前に、計画レビュー用 / 実装用 / diff レビュー用の backend を選択肢で確認し、結果を計画に明記する。親はタスク規模に応じた推奨を各問 1 つ付ける。

#### Claude 親の選択肢

effort 軸(`model_reasoning_effort`)と「codex のモデルは指定しない」注記は Claude 親限定で適用する。effort の目安は基本 xhigh 寄り、簡単・機械的な作業は high / medium に落とす。
...
| codex — effort high | `codex exec` + `-c model_reasoning_effort="high"`。標準的な実装 |
| codex — effort medium | `codex exec` + `-c model_reasoning_effort="medium"`。軽い機械的変更 |
...
| 選択肢 | diff レビューの実行方法 |
|--------|----------|
| codex review — effort xhigh | `codex -a never exec -c model_reasoning_effort="xhigh" review --uncommitted < /dev/null`。重い変更・設計変更向け |
| codex review — effort high | `codex -a never exec -c model_reasoning_effort="high" review --uncommitted < /dev/null`。標準的なレビュー |
...
計画レビューを経た(skip 選択時は未実施の)、3 役の backend / モデルが明記された計画で承認を得る。承認なしで実装に進まない。
...
- Claude 親 + codex: `codex -a never exec -C "<repo>" --sandbox workspace-write -c model_reasoning_effort="<選択>" "<実装指示>" < /dev/null` を Bash `run_in_background` で起動し、完了自動通知で回収する
...
- Claude 親: 複数の `codex exec` / `cursor-agent -p` を Bash `run_in_background` で同時起動する(JOB_DIR・chatId はジョブごとに発行・分離し、出力は各バックグラウンドタスクの TaskOutput で確認する。Codex 親はジョブごとのログ分離を維持する)、または複数 Agent に委譲する。
...
- Claude 親 + codex: `codex -a never exec resume --last "<指摘と修正指示>" < /dev/null` を Bash `run_in_background` で起動し、完了自動通知で回収する
