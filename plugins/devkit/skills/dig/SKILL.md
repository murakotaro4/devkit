...
argument-hint: "[task]"
allowed-tools: ["AskUserQuestion", "ExitPlanMode", "Bash", "Read", "Grep", "Glob", "Agent", "request_user_input", "spawn_agent", "wait_agent", "send_input", "close_agent", "TaskCreate", "TaskUpdate"]
---
...
- Claude 親 + codex: `codex -a never exec --sandbox read-only -c model_reasoning_effort="<選択>" "<計画レビュー指示>" < /dev/null`
...
- Claude 親 + codex: `codex -a never exec -C "<repo>" --sandbox workspace-write -c model_reasoning_effort="<選択>" "<実装指示>" < /dev/null`
- Claude 親 + cursor-agent: `JOB_DIR=$(mktemp -d "${TMPDIR:-/tmp}/devkit-dig-job.XXXXXX") && CHAT_ID="$(cursor-agent create-chat | tr -d '\r\n')" && test -n "$CHAT_ID" && printf '%s\n' "$CHAT_ID" > "$JOB_DIR/chat-id.txt" && echo "JOB_DIR=$JOB_DIR"` で chatId を保存してから `JOB_DIR=<echo された記録済みのパス> && cursor-agent -p --resume "$(cat "$JOB_DIR/chat-id.txt")" --trust --force --model composer-2.5 --workspace "<repo>" --output-format text "<実装指示>" > "$JOB_DIR/cursor-agent.log" 2>&1` を実行する(echo された JOB_DIR のパスを親が記録し、step 8 の修正ループでは記録済みのパスを使う。シェル変数はツール呼び出し間で失われるため。test -n 失敗 = create-chat 失敗として委譲を中止し親が報告する)
...
- Codex 親 + cursor-agent: shell 経由で同じコマンドを実行する

実装委譲の並列化:

- 計画の write_scope を互いに素なジョブに分割できる場合、依存のないジョブは並列に委譲する。
- Claude 親: 複数の `codex exec` / `cursor-agent -p` を Bash `run_in_background` で同時起動する(JOB_DIR・chatId・ログはジョブごとに発行・分離)、または複数 Agent に委譲する。
...
- 5 分超見込みのタスクはバックグラウンド化し、委譲中はユーザーが進捗を確認できる手段を必ず提供する。Claude 親: Monitor 等で出力ログの増分を定期(60〜120 秒)ハートビート表示する。ハートビートには**時刻・ジョブ経過時間・増分**を含め、増分ゼロが続く場合は停滞の継続秒数と推定原因(内部レビュー待ち/長考)を明示する。Codex 親: `wait_agent` で黙って待たず、進捗を定期的にユーザーへ提示する
...
- Claude 親 + codex review: `codex -a never exec -c model_reasoning_effort="<選択>" review --uncommitted < /dev/null`
...
- Claude 親 + codex: `codex -a never exec resume --last "<指摘と修正指示>" < /dev/null`
- Claude 親 / Codex 親 + cursor-agent: `JOB_DIR=<step 6 で記録したパス> && cursor-agent -p --resume "$(cat "$JOB_DIR/chat-id.txt")" --trust --force --model composer-2.5 --workspace "<repo>" --output-format text "<指摘と修正指示>"`(step 6 で保存した chat-id.txt を読む)
