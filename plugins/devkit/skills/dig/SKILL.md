...
| 選択肢 | diff レビューの実行方法 |
|--------|----------|
| codex review — effort medium | `codex -a never exec -c model_reasoning_effort="medium" review --base origin/<default> < /dev/null`(origin なし repo は `--base <default>`)。標準の計画レビュー / diff レビュー(既定) |
| codex review — effort high | `codex -a never exec -c model_reasoning_effort="high" review --base origin/<default> < /dev/null`(origin なし repo は `--base <default>`)。複雑・高リスクな変更向け |
| codex review — effort xhigh | `codex -a never exec -c model_reasoning_effort="xhigh" review --base origin/<default> < /dev/null`(origin なし repo は `--base <default>`)。ユーザー明示、または代表タスクの実測で品質向上を確認できた例外 |
...
- Claude 親 + codex review: worktree 内で `codex -a never exec -c model_reasoning_effort="<選択>" review --base origin/<default> < /dev/null`(origin なし repo は `--base <default>`)を Bash `run_in_background` で起動し、完了自動通知で回収する
