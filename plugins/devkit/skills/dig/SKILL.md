...
| codex review — effort high | `codex -a never exec -c model_reasoning_effort="high" review --uncommitted`。標準的なレビュー |
...
Codex 親で `spawn_agent` / `wait_agent` が使えない場合は、選択肢を再提示する。実装委譲が使えない場合は、承認を得て親実装にフォールバックする。
...
- Claude 親 + codex: `codex -a never exec --sandbox read-only -c model_reasoning_effort="<選択>" "<計画レビュー指示>"`
...
- Claude 親 + codex: `codex -a never exec -C "<repo>" --sandbox workspace-write -c model_reasoning_effort="<選択>" "<実装指示>"`
...
- Claude 親: 複数の `codex exec` を Bash `run_in_background` で同時起動する、または複数 Agent に委譲する。
...
- Claude 親 + codex review: `codex -a never exec -c model_reasoning_effort="<選択>" review --uncommitted`
...
- Claude 親 + codex: `codex -a never exec resume --last "<指摘と修正指示>"`
