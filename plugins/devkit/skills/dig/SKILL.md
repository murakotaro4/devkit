...
実装系 dig では統合方法も 1 問で確認する。既定推奨は「直接統合」、もう一方は「PR 提出」。`command -v gh` が通らない場合、または origin なし repo では PR 選択肢を出さない。ゴール化して自律実行でも統合方法(直接統合 / PR)を確定し、goal 本文へ転記する。非 git repo ではこの質問を出さない。
...
実装 backend で「ゴール化して自律実行」を選んだ場合は、通常の実装委譲の代わりにゴール化引き継ぎを実行する。承認済み計画から目的 / write_scope / 受け入れ条件 / 検証コマンド / 非対象 / worktree 運用・節目 commit・統合方法・version bump を抜き出してゴール本文へ転記する。commit / push は承認済み計画の統合方法として許可された範囲でのみ行うと焼き込み、許可転記がなければ禁止する。実装後の独立レビュー要件も転記必須とする。

ゴール化引き継ぎ:

- Claude 親: `Skill(skill: "devkit:goal-prompt", args: "<dig からの計画引き継ぎである旨 + 目的 / write_scope / 受け入れ条件 / 検証コマンド / 非対象 / worktree 運用・節目 commit・統合方法・version bump / commit・push の扱い / 実装後の独立レビュー要件>")` で起動する。
