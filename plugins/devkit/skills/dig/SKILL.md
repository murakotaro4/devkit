...
4. PR 経路は次の状態機械を順に実行する。
   ...
   2. CI 待機: `gh pr checks <PR番号> --watch` で全チェック完了まで待つ。待機上限は計画に記載した値(既定 30 分)とし、上限到達時は merge せず停止・報告する。PR 作成直後に check が未登録で `no checks reported` エラーになった場合は、数分の猶予を置いて再試行する。
   3. green 判定: `gh pr checks <PR番号> --json bucket,name` の `bucket` を明示判定する。全件 `pass`(`skipping` は許容)のときだけ merge 可、`fail` / `cancel` があれば赤、`pending` があれば待機を継続する。`gh` の API・認証エラーをチェック 0 件の成功と混同せず、エラーなら停止・報告する。チェック 0 件の扱いは step 3 の統合質問で対象 repo の CI 有無を計画へ記載して決め、CI なし repo は checks 待ちを省略して merge 可、CI あり repo で 0 件が続く場合は停止・報告する。
   4. merge: green 判定時の head SHA を `gh pr view <PR番号> --json headRefOid` で記録し、`gh pr merge <PR番号> --merge --match-head-commit <SHA>` を実行する。方式フラグは必ず明示し、repo のルールが squash 等の方式を定める場合はそれを優先する。`--delete-branch` は使わない。merge queue / auto-merge の有効化が要求される branch では、auto-merge を有効化せず停止・報告する(queue 投入は許可範囲外)。merge コマンド失敗時に別方式へ自動で切り替えない。
   5. 完了確認: `gh pr view <PR番号> --json state,mergedAt` で `MERGED` を確認して初めて統合完了とする。
   6. 後始末(順序固定): `MERGED` 確認 → `git fetch origin` → `git merge-base --is-ancestor <branch-tip> origin/<default>` で作業 branch tip が `origin/<default>` の ancestor であることを確認 → `git worktree remove <worktree>` → `git branch -d <branch>` → `git push origin --delete <branch>` で remote branch を削除する。cleanup 失敗は `--force` せず、「統合成功・cleanup 未完了」として残存物のパス・ブランチ名を報告する(merge 失敗と区別する)。
