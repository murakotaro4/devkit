...
4. PR 経路は次の状態機械を順に実行する。
   ...
   6. 後始末(順序固定): 共通で `MERGED` 確認 → `git fetch origin` → ローカルの作業 branch tip を記録し、fetch 済みの `origin/<branch>` tip が検証済み SHA(merge した head)と一致することを確認する。remote tip が不一致なら削除へ進まず、「統合成功・cleanup 未完了」として remote に新 commit が残っている旨を報告する。一致時は merge 方式ごとにローカル削除許可を確認する。merge commit 方式では `git merge-base --is-ancestor <branch-tip> origin/<default>` で ancestor と確認してから、`git worktree remove <worktree>` → `git branch -d <branch>` の順に削除する。repo ルールが squash / rebase 方式を定める場合は ancestor 前提を使わず、`gh pr view <PR番号> --json state,mergedAt,headRefOid` で `state` が `MERGED`、`mergedAt` が非空、`headRefOid` が記録したローカル branch tip と一致することを確認し、その削除許可の根拠を記録してから `git worktree remove <worktree>` → `git branch -D <branch>` の順に削除する。どちらの方式も最後に `git push --force-with-lease=refs/heads/<branch>:<検証済みSHA> origin :refs/heads/<branch>` で期待 tip を明示して remote branch を削除する。必要な確認が取れない場合、remote tip が不一致の場合、または lease 失敗時は remote branch を削除せず「統合成功・cleanup 未完了」とし、remote に新 commit が残っている旨と残存物のパス・ブランチ名を報告する。cleanup 途中の失敗も同区分で報告する(merge 失敗と区別する)。
