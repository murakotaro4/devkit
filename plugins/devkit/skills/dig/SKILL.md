...
- 検証コマンド(テスト・リンタ)
- 非対象(やらないこと)
- 作業ブランチ名(`<type>/<slug>`) / 統合方法(既定は PR 経由(提出 + CI green 確認 + merge)。`command -v gh` が通らない、origin なし repo、または origin が `gh` で扱えない(例: `git -C "<repo>" rev-parse --is-inside-work-tree` 済みの repo で `gh repo view` が失敗する非 GitHub ホスト)場合は直接統合へ自動フォールバックし、その旨を計画に明記する。ユーザーが直接統合を明示した場合はそれに従う。確定は計画作成時に親が `command -v gh`・origin 有無・`gh repo view` の成否を確認して行う) / 最終 commit メッセージ案。PR 経由では対象 repo の CI 有無と CI 待機上限(既定 30 分)も記載する
...
実装系 dig の統合方法は質問しない。既定は PR 経由(提出 + CI green 確認 + merge まで)とする。`command -v gh` が通らない場合、origin なし repo、または origin が `gh` で扱えない場合は直接統合へ自動フォールバックし、その旨を計画に明記する。確定は計画作成時(step 2)に親が `command -v gh`・origin 有無・`gh repo view` の成否を確認して行う。ユーザーが直接統合または統合しないを明示した場合のみそれに従う。自動フォールバックは計画作成時の確定であり、実行中の統合方法切り替えではないため、step 9 の「別の統合方法へ黙って切り替えない」契約は維持する。非 git repo では統合を行わない。ゴール化して自律実行でも同じ規則で統合方法(既定は PR 経由(提出 + CI green 確認 + merge まで))を確定して goal 本文へ転記し、PR 経由では対象 repo の CI 有無と CI 待機上限(既定 30 分)を計画へ記載する。
