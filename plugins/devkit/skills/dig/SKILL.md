...
親エージェント = 深掘り・調査・計画・統括・報告。計画レビュー / 実装 / diff レビューは step 3 で選択した backend に委譲する(実装 backend は commit 禁止)。commit は親が作業ブランチへ節目ごとにパス限定で行い、統合(既定は PR の提出 + CI green 確認 + merge。PR 不可 repo では直接統合の merge + push)まで dig が完遂する。
...
- 検証コマンド(テスト・リンタ)
- 非対象(やらないこと)
- 作業ブランチ名(`<type>/<slug>`) / 統合方法(既定は PR 経由(提出 + CI green 確認 + merge)。`command -v gh` が通らない場合、origin なし repo、または `git remote get-url origin` の URL ホスト名が非 GitHub ホストの場合は直接統合へ自動フォールバックし、その旨を計画に明記する。GitHub ホストの origin で `command -v gh` は通るのに `gh repo view` 等が認証切れ・API 障害・ネットワーク断などで失敗する場合は、直接統合へフォールバックせず停止・報告する。ユーザーが直接統合を明示した場合はそれに従う。確定は計画作成時に親が `command -v gh`・origin 有無・`git remote get-url origin` の URL ホスト名・GitHub ホストでの `gh repo view` 等の運用可否を確認して行う) / 最終 commit メッセージ案。PR 経由では repo 内 CI 設定ファイルと GitHub 側設定(`gh api` で取得できる branch ルール・required status checks 等)を調査して対象 repo の CI 有無を記載し、CI 待機上限(既定 30 分)も記載する
...
実装系 dig の統合方法は質問しない。既定は PR 経由(提出 + CI green 確認 + merge まで)とする。`command -v gh` が通らない場合、origin なし repo、または `git remote get-url origin` の URL ホスト名が非 GitHub ホストの場合は直接統合へ自動フォールバックし、その旨を計画に明記する。GitHub ホストの origin で `command -v gh` は通るのに `gh repo view` 等が認証切れ・API 障害・ネットワーク断などで失敗する場合は、直接統合へフォールバックせず停止・報告する。確定は計画作成時(step 2)に親が `command -v gh`・origin 有無・`git remote get-url origin` の URL ホスト名・GitHub ホストでの `gh repo view` 等の運用可否を確認して行う。ユーザーが直接統合または統合しないを明示した場合のみそれに従う。自動フォールバックは計画作成時の確定であり、実行中の統合方法切り替えではないため、step 9 の「別の統合方法へ黙って切り替えない」契約は維持する。非 git repo では統合を行わない。ゴール化して自律実行でも同じ規則で統合方法(既定は PR 経由(提出 + CI green 確認 + merge まで))を確定して goal 本文へ転記し、PR 経由では repo 内 CI 設定ファイルと GitHub 側設定(`gh api` で取得できる branch ルール・required status checks 等)を調査して対象 repo の CI 有無を計画へ記載し、CI 待機上限(既定 30 分)も記載する。
...
実装 backend で「ゴール化して自律実行」を選んだ場合は、通常の実装委譲の代わりにゴール化引き継ぎを実行する。承認済み計画から目的 / write_scope / 受け入れ条件 / 検証コマンド / 非対象 / worktree 運用・節目 commit・統合契約(PR 経由の場合は CI 判定・merge・失敗時契約、直接統合の場合は直接統合の統合契約を含む)・version bump を抜き出してゴール本文へ転記する。dig 計画で確定済みの統合方法(既定は PR 経由)をそのまま転記し、commit / push は承認済み計画の統合方法に従い、統合しないが明示された場合は禁止すると焼き込む。実装後の独立レビュー要件も転記必須とする。

ゴール化引き継ぎ:

- Claude 親: `Skill(skill: "devkit:goal-prompt", args: "<dig からの計画引き継ぎである旨 + 目的 / write_scope / 受け入れ条件 / 検証コマンド / 非対象 / worktree 運用・節目 commit・統合契約(PR 経由の場合は CI 判定・merge・失敗時契約、直接統合の場合は直接統合の統合契約を含む)・version bump / commit・push の扱い / 実装後の独立レビュー要件>")` で起動する。
