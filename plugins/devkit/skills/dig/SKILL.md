...
| ゴール化して自律実行 | 承認済み計画を `goal-prompt` へ引き継ぎ、step 6 の「ゴール化引き継ぎ」でレビュー済みゴールプロンプト + 起動プロンプト(既定はインライン 1 ブロック)に変換する。dig は起動プロンプト提示で終了し、実装済み diff は作らない |
...
| 実装 | ゴール化して自律実行 | 承認済み計画を `goal-prompt` の契約に従って親がそのまま続行し、レビュー済みゴールプロンプト + 起動プロンプト(既定はインライン 1 ブロック)に変換する。dig は起動プロンプト提示で終了し、実装済み diff は作らない |
...
- `goal-prompt` はレビュー済みゴールプロンプト + 起動プロンプト(既定はインライン 1 ブロック)を作成し、提示したところで終了する。dig もそこで終了し、step 7-9 は実行しない。
