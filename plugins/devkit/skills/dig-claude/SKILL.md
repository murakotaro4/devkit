...
- レートリミット時のみ `REVIEW_FALLBACK_CMD`（`gpt-5.4`）へ切替可。実施時は理由を記録
- レートリミット以外の失敗は停止して原因特定

> **共有 workflow との差異**: 共有 workflow の標準ゲートは `codex -a never exec review --uncommitted` サブコマンド用で fallback は `gpt-5.3-codex`。dig-claude は `codex exec -m` を使用するため、fallback 先は `gpt-5.4` とする（利用先リポジトリ運用に合わせて変更可）。
