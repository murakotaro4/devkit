...
- Phase 1-4: AskUserQuestionTool と plan ファイルで進める
- Phase 5: Plan Mode 内で `codex exec` を使い計画レビューを完了する（下記参照）
- Phase 6-8: ユーザーが通常実行へ進めた後に扱う

### Plan Mode での Phase 5 実行

Plan Mode 内でも `codex exec` は読み取り専用コマンドとして実行可能。Phase 4 でプランファイルを書き終えた後、**ExitPlanMode を呼ぶ前に** Phase 5 計画レビューを完了すること。

手順:
1. Phase 4 のプランファイル完成後、通常の Phase 5 手順（本ファイルの Phase 5 セクション）に従い `codex exec` でレビューを実行する
2. `critical=0` かつ `high=0` になるまで修正→再レビューを繰り返す
3. Phase 5 通過後に ExitPlanMode を呼ぶ

ExitPlanMode 前に Phase 5 が完了していない場合、PreToolUse hook が警告する。
...
つまり **Phase 5 通過後** に `[Task 1]` 以降のタスクが無いまま `Edit` / `Write` / `Agent` / 変更系 `Bash` に入ることは許可しない。
...
2. Phase 6 Tasks 登録後なら、未完了タスクを全て cancel
...
- Phase 5 通過後 に `[Task 1]` 以降を一括登録する
