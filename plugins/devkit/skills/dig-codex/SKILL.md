...
Codex は実行フェーズ（Step 4）を持たないため、タスク完了は将来対応。計画フェーズ完了時:
...
| 並列実行 | agent-parallel / tool-parallel | 非対応（計画フェーズのみ） | Step 4 非対応 |
| REVIEW_GATE_DECOMPOSITION | 対応 | 対応 | codex exec 共通 |
| REVIEW_GATE_PLAN | 対応 | 対応（既存） | 既存機能 |
| REVIEW_GATE_SUBTASK | 対応 | 非対応（Step 4 非対応） | dig-core 契約により適用外 |
| REVIEW_GATE_INTEGRATION | 対応 | 非対応（Step 4 非対応） | 同上 |
