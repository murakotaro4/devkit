...
8 観点で確認し、根拠はできる限り `file:line` 形式で示す。行番号を取れない統計値はコマンド名と集計条件を明記する。
...
- 記憶候補抽出: 現在セッションの in-context 情報だけを根拠に、ユーザーが修正した好み、繰り返し現れる運用ルール、コードから導出できないプロジェクト前提を候補として抽出する。一度きりの作業ログ、原文を読めば足りる情報、実行途中のタスク状態は候補にしない。候補は report-only で提示し、保存・注入は step 6 の承認ゲートを通す。
...
- 観点: 矛盾 / 古い前提 / 曖昧な指示 / 重複 / 危険な自動化 / 参照設計 / テスト・検証 / 記憶候補抽出
- 影響: ...
- 推奨: ...
```
...
正本は devkit リポジトリの `AGENTS.md`「スキル共通契約 > 計画・レポートの 2 層提示」。既存の「## 1. 結論(3件以内)」を第 1 層の承認用サマリーとして兼用し、「## 2. 全体評価」以降を第 2 層の詳細とする。結論だけで step 6 の対応方針を判断できるよう、次の 7 カテゴリを長さではなくカテゴリとして省略せず含める。結論そのものと判断してほしい点は 3 件以内に保つ。

1. 何を / なぜ: 1〜2 文
2. 判断してほしい点: 各項目に推奨を付け、最大 3 件。なければ「なし」
3. 既定からの逸脱・採用した仮定: なければ「既定どおり」
4. 後戻りしにくい操作・外部影響: 削除 / 公開など該当するものだけ。delete candidate は提示のみで、該当しなければ「なし」
5. backend 表: 計画レビュー / 実装 / diff レビューをすべて「適用なし」と明記する
6. 検証: 何を確認できれば監査結果を採用できるかを 1 行
7. 独立レビュー状態: `適用なし` + backend `適用なし`。一般形の値は `実施済み(指摘 N 件反映)` / `skip(理由)` / `適用なし` の 3 値とし、監査レポート自体には適用しない

既定どおりの項目は「既定どおり」に畳み、逸脱と仮定だけを列挙する。backend 表など適用不能な項目も省略せず「適用なし」とする。以下の 11 セクション構成と見出しは変更しない。
...
## 10. 記憶候補(report-only)
| 候補 | 根拠となる現在セッションの in-context 情報 | 候補にする理由 | 保存・注入の承認状況 |
|------|--------------------------------------------|----------------|------------------------|
| ... | ... | ユーザーが修正した好み / 繰り返し現れる運用ルール / コードから導出できないプロジェクト前提 | 未承認 |
