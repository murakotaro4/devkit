...
| Round 4: 運用と実行戦略 | 不在実行時の観測性と実装労働の分け方を決める | 進捗の残し方、権限、途中中断と再開、完了報告の形式(既定は完了レポート節)、実装系のみ委譲先・並列方針・トークン効率方針・worktree 分離・節目 commit。統合方法は質問しない(既定は PR 経由(提出 + CI green 確認 + merge まで)。gh 不在 / origin なし repo は直接統合へ自動フォールバック。直接統合 / 統合しない はユーザー明示時のみ)(codex 経路のモデル / effort は `gpt-5.6-sol` / `medium` 固定のため質問しない) |
...
- git repo の実装系では worktree + 節目 commit を既定推奨とする。実装系ゴールの統合方法は既定で PR 経由とし、質問しない。gh 不在 / origin なし repo では直接統合へ自動フォールバックする。ユーザーが直接統合 / 統合しないを明示した場合のみそれに従う。
...
- 統合方法: <既定は PR 経由(提出 + CI green 確認 + merge まで)。gh 不在 / origin なし repo は直接統合へ自動フォールバック。統合しない(diff を残して停止)はユーザー明示時のみ。統合しないが明示された場合のみ commit / push を行わない>
