...
既定では、ゴール本文の契約(停止条件 3 種・blocker プロトコル・進捗ログ復帰点・終了前読み返し)に従い完走する。達成停止・上限停止・行き詰まり停止のどの停止種別でも `.claude/goal-runs/<確定済みレポート名>` へ完了レポートを書き、停止種別 / 成功条件ごとの達成状況 / 検証コマンド結果 / commit・統合の実施状況(許可転記がない場合は「commit 判断はユーザー」)を最終報告する。
