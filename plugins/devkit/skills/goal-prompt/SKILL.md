...
| Round 4: 運用と実行戦略 | 不在実行時の観測性と実装労働の分け方を決める | 進捗の残し方、権限、途中中断と再開、完了報告の形式(既定は完了レポート節)、実装系のみ委譲先・並列方針・トークン効率方針・worktree 分離・節目 commit。統合方法は質問しない(既定は PR 経由(提出 + CI green 確認 + merge まで)。gh 不在 / origin なし / origin が非 GitHub ホストの repo は直接統合へ自動フォールバック。GitHub ホストで gh が認証・API 障害等により使えない場合はフォールバックせず停止・報告。直接統合 / 統合しない はユーザー明示時のみ)(codex 経路のモデル / effort は `gpt-5.6-sol` / `medium` 固定のため質問しない) |
...
- git repo の実装系では worktree + 節目 commit を既定推奨とする。実装系ゴールの統合方法は既定で PR 経由とし、質問しない。gh 不在 / origin なし / origin が非 GitHub ホストの repo は直接統合へ自動フォールバックする。GitHub ホストで gh が認証・API 障害等により使えない場合はフォールバックせず停止・報告する。ユーザーが直接統合 / 統合しないを明示した場合のみそれに従う。
...
進捗の残し方、権限、途中中断と再開、完了報告の形式を聞く。完了報告の形式は完了レポート節を既定として提示する。実装系では、委譲先、並列方針、トークン効率方針、worktree 分離、節目 commit を確認する。統合方法は質問せず、既定は PR 経由(提出 + CI green 確認 + merge まで)とする。gh 不在 / origin なし / origin が非 GitHub ホストの repo は直接統合へ自動フォールバックする。GitHub ホストで gh が認証・API 障害等により使えない場合はフォールバックせず停止・報告する。ユーザーが直接統合 / 統合しないを明示した場合のみそれに従う。PR 経由では対象 repo の CI 有無とチェック 0 件の扱い、CI 待機上限(既定 30 分)を質問ではなく、repo 内 CI 設定ファイルと GitHub 側設定(`gh api` で取得できる branch ルール・required status checks 等)の調査で確定して goal 本文へ記載する。git repo では worktree + 節目 commit を既定推奨とする。codex 経路のモデル / effort は `gpt-5.6-sol` / `medium` 固定のため質問しない。`spawn_agent` など非対応経路は適用なしとする。確認するのは戦略と契約だけで、実行側の逐一の作業手順は作らない。
