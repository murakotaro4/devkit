...
- PR 経由の統合契約: <対象 repo の CI 有無とチェック 0 件の扱い、数分おきの期限管理付きポーリングと CI 待機上限(既定 30 分)、`bucket` 判定(`pass`・`skipping` のみ merge 可)、green 判定時点で束縛した検証済み SHA を `--match-head-commit` に使い merge 直前の head SHA 再確認で不一致なら停止、merge queue / auto-merge 状態を merge 実行前に検出して state 変更コマンドを実行せず停止、remote branch 削除は期待 tip と検証済み SHA の一致確認 + lease 付き削除とし不一致・lease 失敗なら cleanup 未完了として停止、`gh pr view` の `MERGED` 確認、CI 赤・merge 失敗時は PR を open のまま停止して完了レポートへ PR URL・失敗チェック・残存状態を記載する契約を焼き込む。PR 経由以外では適用なし>
