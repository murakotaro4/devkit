...
- 親が保存した後は `git check-ignore` で ignore が効いているか検証する。効いていない場合は保存を維持したまま、既定では実行直前の通知と最終報告、例外形態では起動プロンプト提示に「git 管理対象になっている」警告を添える(既存 `.gitignore` の内容は変更しない)。
...
| Round 4: 運用と実行戦略 | 不在実行時の観測性と実装労働の分け方を決める | 進捗の残し方、権限、途中中断と再開、完了報告の形式(既定は完了レポート節)、実装系のみ委譲先・並列方針・トークン効率方針・worktree 分離・節目 commit・統合方法(直接統合 / PR / 統合しない)(codex 経路のモデル / effort は `gpt-5.6-sol` / `medium` 固定のため質問しない) |
...
- 統合方法: <直接統合 / PR / 統合しない(diff を残して停止)。許可転記がある場合のみ commit / push を行う>
...
- 書き出し後に起動元 checkout 基準の確定済みパスを `git check-ignore` で検証し、ignore が効いていなければレポート末尾と停止報告へ警告を書く。
- 起動元 checkout 基準の確定済みレポートと同名のファイルは上書きせず、`<basename>-2.md` からの連番にする。
...
commit / push の扱い(承認済み計画の統合方法に従う。許可転記がなければ禁止)と、repo ルールが独立レビューを必須とする場合の実装後レビュー要件、どの停止種別でも書き出す完了レポート要件は転記必須項目とする。承認済み計画の worktree 運用・節目 commit・統合方法・version bump も「実行戦略」へ転記する。差分確認後は通常フローの組み立て + セルフチェック 10 項目、独立レビューを経て、既定では step 8 の実行移行へ進む。dig の計画承認が実行許可の出所であり、承認質問を二重化しない。
...
進捗の残し方、権限、途中中断と再開、完了報告の形式を聞く。完了報告の形式は完了レポート節を既定として提示する。実装系では、委譲先、並列方針、トークン効率方針、worktree 分離、節目 commit、統合方法(直接統合 / PR / 統合しない)を確認する。git repo では worktree + 節目 commit を既定推奨とし、統合の明示回答がなければ diff と完了レポートを残して停止して commit 判断をユーザーへ戻す。codex 経路のモデル / effort は `gpt-5.6-sol` / `medium` 固定のため質問しない。`spawn_agent` など非対応経路は適用なしとする。確認するのは戦略と契約だけで、実行側の逐一の作業手順は作らない。

### 6. 組み立て + セルフチェック

プロンプトテンプレートへ回答を統合し、セルフチェック 10 項目を満たすまで修正する。既定経路では、起動元 checkout を確定し、ゴールファイル・進捗ログ・完了レポートの 3 パスをその checkout の `.claude/goal-runs/` 基準の具体パスへ解決してテンプレート本文へ焼き込む。完了レポート名は、保存済みゴールならゴールファイルの末尾の `-goal` サフィックスを除いた basename、未保存なら短い名前から作る `YYYY-MM-DD-<slug>.md` として具体名をテンプレート本文へ焼き込む。例外形態の現セッション `/goal` では、親が objective 全文(ヘッダー・空行・本文込み、`/goal` プレフィックスを除く)を Unicode 文字数で数え、4,000 字以内ならインライン、4,001 字以上なら fallback と確定する。Codex 貼付けは字数に関わらず常に全文 1 ブロックとし、ファイル fallback へ分岐しない。既定の現セッション自動実行では字数による受け渡し判定を行わない。
...
既定では、独立レビュー反映後の完成プロンプト全文を通知として提示し、承認待ちなしで進む。親がゴール本文全文を `.claude/goal-runs/YYYY-MM-DD-<slug>-goal.md` へ保存し、ディレクトリと `*` 1 行の `.gitignore` の ensure・同名時の連番・`git check-ignore` 検証を行う。これはコンテキスト圧縮後の復帰点であり、保存に失敗した場合は実行を開始せず停止・報告する。保存成功後、「実行モード: 不在自律実行」宣言を自分に適用して直ちに実行を開始する。実行開始の宣言に保存済みゴールファイルの具体パスを明記し、コンテキスト圧縮後はまずそのファイルを読み直してから進捗ログ冒頭の復帰点を読む、という順序を実行契約として自分に課す。実行直前の全文提示は通知手段であり、安全ゲートは、dig 経由では承認済み計画、直起動の書き込み・破壊的操作・外部状態変更では Round 3、直起動の commit・統合では実装系 Round 4 の明示回答から転記した操作別の許可と停止条件が担う。
