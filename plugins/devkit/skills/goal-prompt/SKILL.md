...
- 既定では step 8 で実行へ移行する。cron 登録・`/schedule` 登録はどの step でも行わない。作成フェーズは read-only、実行フェーズはゴール本文の契約に従う。直起動の書き込み・破壊的操作・外部状態変更は Round 3 の明示回答から転記された許可がある場合だけ行う。commit / push・統合は goal 本文に記載された統合方法(既定は PR 経由。dig 経由では承認済み計画の統合方法)に従う。統合しないが明示された場合は commit / push を行わない。無回答・曖昧な回答から統合以外の書き込み・破壊的操作・外部状態変更の許可を推定しない。
...
| Round 4: 運用と実行戦略 | 不在実行時の観測性と実装労働の分け方を決める | 進捗の残し方、権限、途中中断と再開、完了報告の形式(既定は完了レポート節)、実装系のみ委譲先・並列方針・トークン効率方針・worktree 分離・節目 commit。統合方法は質問しない(既定は PR 経由(提出 + CI green 確認 + merge まで)。gh 不在 / origin なし / origin が gh で扱えない repo は直接統合へ自動フォールバック。直接統合 / 統合しない はユーザー明示時のみ)(codex 経路のモデル / effort は `gpt-5.6-sol` / `medium` 固定のため質問しない) |
...
- git repo の実装系では worktree + 節目 commit を既定推奨とする。実装系ゴールの統合方法は既定で PR 経由とし、質問しない。gh 不在 / origin なし / origin が gh で扱えない repo は直接統合へ自動フォールバックする。ユーザーが直接統合 / 統合しないを明示した場合のみそれに従う。
...
- 統合方法: <既定は PR 経由(提出 + CI green 確認 + merge まで)。gh 不在 / origin なし / origin が gh で扱えない repo は直接統合へ自動フォールバック。統合しない(diff を残して停止)はユーザー明示時のみ。統合しないが明示された場合のみ commit / push を行わない>
...
commit / push の扱い(承認済み計画の統合方法に従う。統合しないが明示された場合は禁止)と、repo ルールが独立レビューを必須とする場合の実装後レビュー要件、どの停止種別でも書き出す完了レポート要件は転記必須項目とする。承認済み計画で確定済みの統合方法(既定は PR 経由)をそのまま転記し、worktree 運用・節目 commit・統合契約(PR 経由の場合は CI 判定・merge・失敗時契約、直接統合の場合は直接統合の統合契約を含む)・version bump も「実行戦略」へ転記する。差分確認後は通常フローの組み立て + セルフチェック 10 項目、独立レビューを経て、既定では step 8 の実行移行へ進む。dig の計画承認が実行許可の出所であり、承認質問を二重化しない。
...
進捗の残し方、権限、途中中断と再開、完了報告の形式を聞く。完了報告の形式は完了レポート節を既定として提示する。実装系では、委譲先、並列方針、トークン効率方針、worktree 分離、節目 commit を確認する。統合方法は質問せず、既定は PR 経由(提出 + CI green 確認 + merge まで)とする。gh 不在 / origin なし / origin が gh で扱えない repo は直接統合へ自動フォールバックし、ユーザーが直接統合 / 統合しないを明示した場合のみそれに従う。PR 経由では対象 repo の CI 有無とチェック 0 件の扱い、CI 待機上限(既定 30 分)を質問ではなく親の調査(CI 設定ファイルの有無確認等)で確定して goal 本文へ記載する。git repo では worktree + 節目 commit を既定推奨とする。codex 経路のモデル / effort は `gpt-5.6-sol` / `medium` 固定のため質問しない。`spawn_agent` など非対応経路は適用なしとする。確認するのは戦略と契約だけで、実行側の逐一の作業手順は作らない。

### 6. 組み立て + セルフチェック

プロンプトテンプレートへ回答を統合し、セルフチェック 10 項目を満たすまで修正する。PR 経由の実装系ゴールでは、対象 repo の CI 有無とチェック 0 件の扱い / 数分おきの期限管理付きポーリングと CI 待機上限(既定 30 分) / `bucket` の `pass`・`skipping` のみ merge 可とする判定 / green 判定時点で束縛した検証済み SHA を `--match-head-commit` に使い merge 直前の head SHA 再確認で不一致なら停止 / merge queue・auto-merge 状態を merge 実行前に検出して state 変更コマンドを実行せず停止 / remote branch存在確認は `git ls-remote --heads origin refs/heads/<branch>` の成功かつ空結果だけをGitHub側削除済みと扱いAPI・認証・通信エラーや複数結果はcleanup未完了で停止 / remote branch 存在時は期待 tip と検証済み SHA の一致確認 + lease 付き削除とし不一致・lease 失敗なら cleanup 未完了として停止 / remote branch が GitHub 側で削除済みなら PR の `headRefOid` と検証済み SHA の一致確認後に remote 削除完了扱いでローカルcleanupを続行 / `gh pr view` の `MERGED` 確認で統合完了 / CI 赤・merge 失敗時は PR を open のまま停止して完了レポートへ PR URL・失敗チェック・残存状態を記載、の統合契約を「実行戦略」へ焼き込む。直接統合では、主 worktree の clean・default branch・origin 非 diverged 確認 / `merge --ff-only` → push / push reject 時の fetch + rebase + 検証再実行 + version 再計算 / origin なし repo の push なし完了 / 統合成功後の worktree・ローカル branch cleanup / 失敗時は別の統合方法へ切り替えず差分と作業状態を保持して停止、の直接統合の統合契約を「実行戦略」へ焼き込む。既定経路では、起動元 checkout(非 git ではスキル起動時の cwd)の `.claude/goal-runs/` にある既存ファイルを組み立て時に確認し、衝突しない最終 basename(必要なら連番込み)を先に確定する。その basename からゴールファイル・進捗ログ・完了レポートの 3 パスを基準ディレクトリ内の具体パスへ解決してテンプレート本文へ焼き込む。完了レポート名は、保存済みゴールならゴールファイルの末尾の `-goal` サフィックスを除いた basename、未保存なら短い名前から作る `YYYY-MM-DD-<slug>.md` として具体名をテンプレート本文へ焼き込む。例外形態の現セッション `/goal` では、親が objective 全文(ヘッダー・空行・本文込み、`/goal` プレフィックスを除く)を Unicode 文字数で数え、4,000 字以内ならインライン、4,001 字以上なら fallback と確定する。Codex 貼付けは字数に関わらず常に全文 1 ブロックとし、ファイル fallback へ分岐しない。既定の現セッション自動実行では字数による受け渡し判定を行わない。
