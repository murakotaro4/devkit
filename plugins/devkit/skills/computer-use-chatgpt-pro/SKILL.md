...
description: "macOS の Codex Desktop Computer Use で、可視ブラウザ上の ChatGPT Pro に単発相談する明示依頼で使用する。「Computer UseでChatGPT Proに質問」「Codex Desktopの可視ブラウザでGPT Proに相談」など。汎用のChatGPT Pro検索・調査は既存のgpt-proを使う。agent-browser/CDP/headless/DOM eval は使わない。"
...
Codex Desktop の Computer Use で macOS の実ブラウザを操作し、ChatGPT Pro に相談する。
...
```text
あなたは Codex が可視ブラウザ経由で相談している外部アドバイザーです。
