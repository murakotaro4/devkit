...
description: "macOS の Codex Desktop Computer Use で ChatGPT デスクトップアプリの ChatGPT Pro に単発相談する明示依頼で使用する。「Computer UseでChatGPT Proに質問」「ChatGPTアプリでGPT Proに相談」など。汎用のChatGPT Pro検索・調査は既存のgpt-proを使う。agent-browser/CDP/headless/DOM eval は使わない。"
...
Codex Desktop の Computer Use で ChatGPT デスクトップアプリを操作し、ChatGPT Pro に相談する。
回答は外部アドバイスとして扱い、Codex 側で検証・要約してユーザーに返す。

## 適用条件

- macOS の Codex Desktop で、Computer Use plugin が使えること。
- ChatGPT デスクトップアプリがインストール済みであること。
- ユーザー本人の ChatGPT アカウントで、Pro モデルが利用可能であること。
- Computer Use が ChatGPT アプリを操作できること。

macOS 以外、Computer Use が使えない環境、ChatGPT アプリが使えない環境、またはログイン操作をユーザーが完了できない場合は停止する。

## 禁止境界

- `agent-browser`、CDP、headless Chrome、Playwright DOM eval、ChatGPT DOM の直接取得を使わない。
- cookie、localStorage、sessionStorage、アプリ内部データ、パスワード、トークンを読まない・保存しない・共有しない。
...
操作前には必ず ChatGPT アプリで `mcp__computer_use__.get_app_state` を呼び、画面とアクセシビリティツリーを確認する。
...
ChatGPT デスクトップアプリを使う。Google Chrome や Safari は使わない。

```bash
open -a "ChatGPT"
```

アプリ操作は Computer Use で行う。アプリが見つからない場合は停止し、ユーザーにインストールまたは起動を依頼する。ログイン画面が出た場合はユーザーに手動ログインを依頼し、認証情報を入力・保存・取得しない。
...
長い回答は、画面上の Copy ボタンを Computer Use で押し、`pbpaste` で取得してよい。ただしクリップボードから読む対象は ChatGPT の回答だけに限定し、認証情報やアプリ内部状態は読まない。
